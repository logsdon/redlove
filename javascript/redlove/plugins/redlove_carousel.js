	...
	;(function ( $, window, document, undefined )// Begin function closure; avoid collisions
	{
		...
		*/
		...
		proto.default_options = {
			...
			nav : true,
			nav_selector : '',
			nav_default_class : 'redlove_carousel-nav',
		...
		*/
		proto.init = function ( el, options )
		{
			...
			if ( inst.options.paging )
			{
				inst.paging_template = '\
					<div class="' + inst.options.paging_default_class + '">\
						<ol>\
							<li><a href="#"></a></li>\
						</ol>\
					</div>\
				';
				inst.$paging = ( inst.options.paging_selector.length > 0 ) ? $(inst.options.paging_selector) : $(inst.paging_template).prependTo(inst.$carousel);
			...
			{
				inst.nav_template = '\
					<div class="' + inst.options.nav_default_class + '">\
						<a href="#" class="redlove_carousel-nav_prev"></a>\
						<a href="#" class="redlove_carousel-nav_next"></a>\
					</div>\
				';
				inst.$nav = ( inst.options.nav_selector.length > 0 ) ? $(inst.options.nav_selector) : $(inst.nav_template).prependTo(inst.$carousel);
			...
			if ( inst.options.paging )
			{
				...
				.on('click.' + proto.name, 'a', function ( event )
				{
					...
					var paging_index = $(original_element).parent().index();
